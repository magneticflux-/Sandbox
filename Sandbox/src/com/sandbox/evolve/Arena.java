...
public class Arena {
	...
	public static void main1(final String[] args) {
		...
		for (int i = 0; i < 10; i++) {
			...
			a.addFighter(a.new Fighter(n.getDeepCopy(), FastMath.random() * 400 + 200, FastMath.random() * 400 + 200, FastMath.random() * FastMath.PI * 2, true, false, a));
		}
	...
	public void paint(final Graphics g) {
		...
		for (int i = 0; i < this.fighters.size(); i++) {
			g2d.setColor(Color.BLACK);
			g2d.drawString("Fighter " + (i + 1) + ": " + this.fighters.get(i).getScore().bigDecimalValue() + "pts. " + this.fighters.get(i).getNumVisibleEnemyFighters() + " visible fighters. " + this.fighters.get(i).getNumVisibleEnemyProjectiles() + " visible projectiles. " + this.fighters.get(i).shootDelay, 50, 20 + i * g2d.getFontMetrics().getHeight());

			g2d.setColor(this.fighters.get(i).team);
			g2d.fillRect(50 - g2d.getFontMetrics().getHeight(), 20 + (i - 1) * g2d.getFontMetrics().getHeight(), g2d.getFontMetrics().getHeight(), g2d.getFontMetrics().getHeight());
		}
	}
		...
		if (this.age > this.maxAge && this.maxAge > 0)
			...
			for (final Iterator<Projectile> i = this.projectiles.iterator(); i.hasNext(); ) {
				...
				if (!this.bounds.contains(p.getX(), p.getY())) {
					i.remove();
					if (p.getOwner() != null)
						p.getOwner().decrementScore(new BigFraction(1, 2));
				} else
					for (final Fighter f : this.fighters)
						if (MathUtil.distance(p.getX(), p.getY(), f.getX(), f.getY()) <= Projectile.RADIUS + Fighter.RADIUS) {
							i.remove();
							if (p.getOwner() != null)
								p.getOwner().incrementScore(BigFraction.ONE);
	...
	public class Fighter extends Projectile {
		...
		public final Color team = new Color((int) FastMath.round(FastMath.random() * 255), (int) FastMath.round(FastMath.random() * 255), (int) FastMath.round(FastMath.random() * 255));
		private final Arena arena;
		public double fov;
		public boolean isShooting = false;
		public double range;
		private FeedforwardNetwork brain;
		private BigFraction score;
		private int shootDelay = 0;
		private boolean isCheating;

		public Fighter(final FeedforwardNetwork brain, final double x, final double y, final double angle, final boolean isRobot, final boolean isCheating, final Arena arena) {
			super(x, y, null);

			if (this.brain != null && !(Integer.parseInt(brain.getLayout().substring(0, 1)) >= 3 && Integer.parseInt(brain.getLayout().substring(brain.getLayout().length() - 1, brain.getLayout().length())) >= 6))
		...
		public Rectangle getBoundingBox() {
			return new Rectangle((int) FastMath.round(this.x - Fighter.RADIUS), (int) FastMath.round(this.y - Fighter.RADIUS), (int) FastMath.round(2 * Fighter.RADIUS), (int) FastMath.round(2 * Fighter.RADIUS));
		}
		...
		public void paint(final Graphics g) {
			...
			if (this.isShooting) {
				...
				g2d.fillOval((int) FastMath.round(this.x - Fighter.RADIUS - 2), (int) FastMath.round(this.y - Fighter.RADIUS - 2), (int) FastMath.round(2 * Fighter.RADIUS + 4), (int) FastMath.round(2 * Fighter.RADIUS + 4));
			}

			g2d.setColor(this.team);
			g2d.fillOval((int) FastMath.round(this.x - Fighter.RADIUS), (int) FastMath.round(this.y - Fighter.RADIUS), (int) FastMath.round(2 * Fighter.RADIUS), (int) FastMath.round(2 * Fighter.RADIUS));
			...
			g2d.drawArc((int) FastMath.round(this.x - this.range), (int) FastMath.round(this.y - this.range), (int) FastMath.round(this.range * 2), (int) FastMath.round(this.range * 2), -(int) FastMath.round(FastMath.toDegrees(this.angle - this.fov / 2)), -(int) FastMath.round(FastMath.toDegrees(this.fov)));
		}
		...
		public void updatePosition() {
			...
			if (FastMath.abs(this.angle) >= FastMath.PI * 2)
				this.angle = (FastMath.PI * 2 + this.angle) % (FastMath.PI * 2);
			...
			if (this.shootDelay > 0)
				...
				this.arena.addProjectile(new Projectile(p1.getX() + this.getX(), p1.getY() + this.getY(), p2.getX() + this.xV, p2.getY() + this.yV, this.angleV, this));
			}
		}
		...
		public void react() {
			if (this.isRobot && this.brain != null) {
				if (this.isCheating) {
					this.reactTo(new double[]{this.angle - MathUtil.angleTo(this.getPoint(), arena.getOtherFighter(this).getPoint()), this.getNumVisibleEnemyProjectiles(), this.getNumVisibleEnemyFighters()});
				} else {
		...
		public void reactTo(final double[] enviroment) {
			...
			if (this.fov + reaction[4] / 10 >= 0 && this.fov + reaction[4] / 10 <= FastMath.PI)
				this.fov += reaction[4] / 10;
			if (this.range + reaction[5] * 10 >= 50)
				this.range += reaction[5] * 10;
		}

		public int getNumVisibleEnemyProjectiles() {
			int sum = 0;
			for (final Projectile p : this.arena.getProjectiles())
				if (MathUtil.angleTo(this.getPoint(), p.getPoint()) - this.angle < this.fov / 2 && MathUtil.distance(this.getPoint(), p.getPoint()) < this.range && this != p.getOwner())
					sum++;
			return sum;
		}
		...
		public int getNumVisibleEnemyFighters() {
			...
			for (final Fighter f : this.arena.getFighters()) {
				if (!this.isRobot && f != this)
					System.out.printf("AngleTo: %.4f Angle: %.4f Diff: %.4f\n", MathUtil.angleTo(this.getPoint(), f.getPoint()), this.angle, FastMath.abs(MathUtil.angleTo(this.getPoint(), f.getPoint()) - this.angle));

				if (FastMath.abs(MathUtil.angleTo(this.getPoint(), f.getPoint()) - this.angle) < this.fov / 2 && MathUtil.distance(this.getPoint(), f.getPoint()) < this.range && f != this)
					sum++;
			}
	...
	public class Projectile {
		...
		public Rectangle getBoundingBox() {
			return new Rectangle((int) FastMath.round(this.x - Projectile.RADIUS), (int) FastMath.round(this.y - Projectile.RADIUS), (int) FastMath.round(2 * Projectile.RADIUS), (int) FastMath.round(2 * Projectile.RADIUS));
		}
		...
		public void paint(final Graphics g) {
			...
			g.fillOval((int) FastMath.round(this.x - Projectile.RADIUS), (int) FastMath.round(this.y - Projectile.RADIUS), (int) FastMath.round(2 * Projectile.RADIUS), (int) FastMath.round(2 * Projectile.RADIUS));

			final PolarPoint[] points = new PolarPoint[]{new PolarPoint(Projectile.RADIUS, this.angle), new PolarPoint(Projectile.RADIUS, this.angle + FastMath.PI / 2), new PolarPoint(Projectile.RADIUS, this.angle + FastMath.PI), new PolarPoint(Projectile.RADIUS, this.angle + 3 * FastMath.PI / 2)};
		...
		public void updatePosition() {
			if (!this.isControlled) {
				final PolarPoint p1 = new PolarPoint(Arena.AIR_DENSITY * FastMath.sqrt(FastMath.pow(this.xV, 2) + FastMath.pow(this.yV, 2)) * MathUtils.TWO_PI * this.angleV * FastMath.pow(Projectile.RADIUS, 2), FastMath.atan2(this.yV, this.xV) + FastMath.PI / 2);
				final PolarPoint p2 = new PolarPoint(-(Arena.SPHERE_CD * 0.5 * Arena.AIR_DENSITY * (FastMath.pow(this.xV, 2) + FastMath.pow(this.yV, 2)) * FastMath.pow(Projectile.RADIUS, 2) * FastMath.PI), FastMath.atan2(this.yV, this.xV));
				this.xV += p1.getX() + p2.getX();
				this.yV += p1.getY() + p2.getY();
			}
			this.x += this.xV;
