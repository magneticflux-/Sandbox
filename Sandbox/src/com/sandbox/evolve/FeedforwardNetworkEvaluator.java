...
public class FeedforwardNetworkEvaluator implements FitnessEvaluator<FeedforwardNetwork>, EvolutionObserver<FeedforwardNetwork> {
	...
	public double getFitness(final FeedforwardNetwork candidate, final List<? extends FeedforwardNetwork> population) {
		...
		if (FeedforwardNetworkEvolve.FIGHT_SELF && !FeedforwardNetworkEvolve.USE_FILE_FOR_OPPONENT)
			this.previousBest = candidate;

		a.addFighter(a.new Fighter(candidate, r.nextDouble() * a.getBounds().getWidth(), r.nextDouble() * a.getBounds().getHeight(), r.nextDouble() * FastMath.PI * 2, true, false, a));
		a.addFighter(a.new Fighter(this.previousBest, r.nextDouble() * a.getBounds().getWidth() + a.getBounds().getMinX(), r.nextDouble() * a.getBounds().getHeight() + a.getBounds().getMinY(), r.nextDouble() * FastMath.PI * 2, true, false, a));
