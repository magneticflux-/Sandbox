	...
	public static void main(final String[] args)
	{
		...
		frame.add(new JPanel()
		{
			...
			public void paintComponent(final Graphics g)
			{
				...
				final Future<Complex>[][] pixels = new Future[height][width];
						...
						try
				{
							bi.setRGB(
									col,
									row,
									HSLColor.toRGB((float) (-pixels[row][col].get().getArgument() / (2 * FastMath.PI)), 1,
											(float) (FastMath.log1p(pixels[row][col].get().abs()) % 1)));
