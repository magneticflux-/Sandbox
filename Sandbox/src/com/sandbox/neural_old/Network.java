...
public class Network {
	...
	// Example "2 2 1" would create a neural net with 2 inputs, two layers of two neurons each, and then a final neuron.
	{
		...
		int o = 0;
		while (s.hasNext()) {// For each layer
	...
	public Network[] breed(final Network mate) {
		...
		for (int layer = 0; layer < this.neurons.length; layer++)
			for (int neuron = 0; neuron < this.neurons[layer].length; neuron++) { // For each neuron in each layer
				...
				for (int weight = 0; weight < crossoverPoint; weight++) {
					...
					if (this.r.nextDouble() < Config.MUTATION_RATE)
						n[0].neurons[layer][neuron].mutateWeight(weight);
					if (this.r.nextDouble() < Config.MUTATION_RATE)
						n[1].neurons[layer][neuron].mutateWeight(weight);
				}
				...
				n[0].neurons[layer][neuron].setWeight(crossoverPoint, skew * this.neurons[layer][neuron].getWeight(crossoverPoint) + (1 - skew) * mate.neurons[layer][neuron].getWeight(crossoverPoint));
				n[1].neurons[layer][neuron].setWeight(crossoverPoint, (1 - skew) * this.neurons[layer][neuron].getWeight(crossoverPoint) + skew * mate.neurons[layer][neuron].getWeight(crossoverPoint));
	...
	public void inspectNet() {
		for (int layer = 0; layer < this.neurons.length; layer++) {
			...
			for (int neuron = 0; neuron < this.neurons[layer].length; neuron++)
				System.out.println("    Neuron " + neuron + " has weights of " + this.neurons[layer][neuron].getWeights() + " and an output bias of " + this.neurons[layer][neuron].getBias());
		}
	}
