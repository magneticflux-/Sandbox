...
public class FeedforwardNetwork extends NeuralNet {
		...
		for (int layer = 0; layer < n.getLayers().size(); layer++)
			for (int node = 0; node < n.getLayers().get(layer).getNodes().size(); node++)
				if (n.getLayers().get(layer).getNode(node) instanceof NeuronNode)
					((NeuronNode) n.getLayers().get(layer).getNode(node)).setWeights((List<Double>) ((ArrayList<Double>) ((NeuronNode) this.getLayers().get(layer).getNode(node)).getWeights()).clone());
		return n;
	}
	...
	public BufferedImage getGraphImage() {
		...
		final DOTExporter<AbstractNode, DefaultWeightedEdge> exporter = new DOTExporter<AbstractNode, DefaultWeightedEdge>(new VertexNameProvider<AbstractNode>() {
			private int iter = 0;
		...
		if (Double.parseDouble(System.getProperty("java.specification.version")) < 1.8) {
			System.out.println("Java version is not supported. Functionality is in reduced mode. Your version: " + Double.parseDouble(System.getProperty("java.specification.version")));
	...
	public DirectedWeightedPseudograph<AbstractNode, DefaultWeightedEdge> getGraphView() {
		final DirectedWeightedPseudograph<AbstractNode, DefaultWeightedEdge> graph = new DirectedWeightedPseudograph<AbstractNode, DefaultWeightedEdge>(DefaultWeightedEdge.class);
	...
	public double[] evaluate(final double[] inputs) {
		if (inputs.length != this.inputs.getNodes().size())
			throw new IllegalArgumentException("Invalid input size! Cannot evaluate neural net. Input size: " + inputs.length + " Neural net input size: " + this.inputs.getNodes().size());
